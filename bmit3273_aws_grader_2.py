...
Run inside AWS Academy Learner Lab CloudShell:
    python3 bmit3273_aws_grader_2.py

The grader performs read-only AWS API checks only. Question 3 verifies the EFS
configuration (file system, mount target, security group, NFS rule) at the
console/API level. No SSH or Systems Manager access to the instance is required.
"""
...
def main():
    ...
    # Question 3 (console/API-only: EFS + mount target + security group + NFS rule)
    ...
    try:
        ...
        if target_fs:
            ...
            q3 += award("EFS mount target is in the required VPC", 3) if in_vpc else fail("EFS mount target is in the required VPC", 3)
            ...
            if target_efs_sg:
                ...
                for mt in mount_targets:
                    ...
                    if target_efs_sg.get("GroupId") in mt_sgs:
                        attached_to_mt = True
                        break
                q3 += award("EFS security group attached to mount target", 3) if attached_to_mt else fail("EFS security group attached to mount target", 3)
                ...
                q3 += award("NFS TCP 2049 access permits only the web security group", 3) if nfs_ok else fail("NFS TCP 2049 access from web security group", 3)
            else:
                q3 += fail(f"Exact EFS security group name: {expected['efs_sg']}", 2)
                q3 += fail("EFS security group attached to mount target", 3)
            ...
            for description, points in [
                ...
                ("EFS tag Project = BMIT3273", 2), ("EFS mount target is in required VPC", 3),
                ("Available EFS mount target is in required subnet", 3),
                (f"Exact EFS security group name: {expected['efs_sg']}", 2),
                ("EFS security group attached to mount target", 3),
