...
if [ -n "$lt_check" ]; then
  ...
  latest_version=$(aws ec2 describe-launch-templates --region "$REGION" --launch-template-names "$lt_name" --query 'LaunchTemplates[0].DefaultVersionNumber' --output text)

  lt_data=$(aws ec2 describe-launch-template-versions --region "$REGION" --launch-template-id "$lt_id" --versions "$latest_version")
  instance_type=$(echo "$lt_data" | jq -r '.LaunchTemplateVersions[0].LaunchTemplateData.InstanceType // empty')
  ...
  if [ -n "$ec2_instance" ]; then
    [[ $MODE == "teacher" ]] && echo "✅ EC2 instance launched from template found: $ec2_instance"
...
if [ -n "$asg_check" ]; then
  ...
  asg_config=$(aws autoscaling describe-auto-scaling-groups --region "$REGION" --auto-scaling-group-names "$asg_name" --query "AutoScalingGroups[0].[MinSize,DesiredCapacity,MaxSize]" --output text)
  if echo "$asg_config" | grep -q -E "^1\s+2\s+4"; then
    [[ $MODE == "teacher" ]] && echo "✅ ASG scaling config correct (Min=1, Desired=2, Max=4)"
    total_score=$((total_score + 5))
  else
    [[ $MODE == "teacher" ]] && echo "❌ ASG scaling config incorrect (found $asg_config)"
  fi
else
