...
# Mode: default student, use --teacher for verbose output
MODE="student"
if [[ "$1" == "--teacher" ]]; then
  MODE="teacher"
fi
...
lt_check=$(aws ec2 describe-launch-templates --region "$REGION" \
    --query "LaunchTemplates[?LaunchTemplateName=='$lt_name']" --output text)
if [ -n "$lt_check" ]; then
    [[ $MODE == "teacher" ]] && echo "✅ Launch Template '$lt_name' found"
    total_score=$((total_score + 7))

    # Check instance type and user data
    latest_version=$(aws ec2 describe-launch-templates --launch-template-names "$lt_name" --region "$REGION" \
        --query "LaunchTemplates[0].LatestVersionNumber" --output text)
    version_data=$(aws ec2 describe-launch-template-versions --launch-template-name "$lt_name" \
        --versions "$latest_version" --region "$REGION")
    ...
    # Check for running EC2 instance launched from this template
    ...
    if [ -n "$instance_check" ]; then
        [[ $MODE == "teacher" ]] && echo "✅ EC2 instance launched from template is running: $instance_check"
        total_score=$((total_score + 10))
    else
...
tg_check=$(aws elbv2 describe-target-groups --region "$REGION" \
    --query "TargetGroups[?TargetGroupName=='$tg_name']" --output text)
if [ -n "$tg_check" ]; then
...
asg_check=$(aws autoscaling describe-auto-scaling-groups --region "$REGION" \
    --query "AutoScalingGroups[?AutoScalingGroupName=='$asg_name']" --output json)
if [ -n "$asg_check" ] && [ "$asg_check" != "[]" ]; then
    [[ $MODE == "teacher" ]] && echo "✅ ASG '$asg_name' exists"
    total_score=$((total_score + 5))

    min_size=$(echo "$asg_check" | jq -r '.[0].MinSize')
    desired_size=$(echo "$asg_check" | jq -r '.[0].DesiredCapacity')
    max_size=$(echo "$asg_check" | jq -r '.[0].MaxSize')

    if [[ "$min_size" == 1 && "$desired_size" == 2 && "$max_size" == 4 ]]; then
        [[ $MODE == "teacher" ]] && echo "✅ ASG scaling configuration correct"
        total_score=$((total_score + 5))
    else
...
rds_check=$(aws rds describe-db-instances --region "$REGION" \
    --query "DBInstances[?DBInstanceIdentifier=='$rds_name']" --output text)
if [ -n "$rds_check" ]; then
